            ...
            if selected_md_file:
                try:
                    ...
                    if not md_text.strip():
                        st.error(f"File {selected_md_file.name} is empty.")
                    else:
                    ...
                    st.error(f"Error loading {selected_md_file.name}: {e}")
...
if out:
    ...
    with tab_preview:
        ...
        if not final_md:
            st.info("No markdown content to display. Generate a new blog or load a past blog.")
        else:
            try:
                render_markdown_with_local_images(final_md)
            except Exception as e:
                st.error(f"Error rendering markdown: {e}")
                st.text(final_md[:500])  # Show raw content for debugging
